...
TOOLS = [
    ...
    },
    {
        "name": "travel_time",
        "description": (
            "Estimate distance and travel time between two places by one mode. For "
            "driving it accounts for traffic at the departure time, so you can tell "
            'Ben when to leave for an event. Read-only. You can use "home" or "work" '
            "for origin/destination — they resolve to Ben's saved addresses. If you "
            "only know a rough place name, resolve it with find_place first. To "
            "compare driving against public transport, call this once per mode."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "mode": {
                    ...
                    "description": (
                        "How Ben is travelling. Defaults to driving; ask Ben if he "
                        "hasn't said and a long trip could plausibly be by train."
                    ),
                },
                "depart_at": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": (
            "Plan a route to a place by one mode and attach it to an existing event: "
            "sets the event's location and adds a Google Maps directions link, the "
            "travel time and a 'leave by' time to its description. Driving is "
            'traffic-aware. You can use "home"/"work" for origin/destination. '
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "mode": {
                    ...
                    "enum": ["driving", "walking", "bicycling", "transit"],
                    "description": (
                        "How Ben will travel — driving or transit (also walking / "
                        "bicycling). If he hasn't said and you can't infer it, ask him "
            ...
            "required": ["event_id", "calendar_id", "origin", "destination", "mode"],
        },
    },
    {
...
def execute_tool(name: str, inp: dict, state: dict, flags: dict) -> str:
    ...
    if name == "add_route_to_event":
        ...
        if event["jarvis"]:
            ...
            return f"Added the {mode} route to '{event['summary']}'.\n\n{block}"
        label = f"Add {mode} route to '{event['summary']}' ({location})"
        pending.append(
            {
                "id": uuid4().hex,
                "action": "modify_event",
